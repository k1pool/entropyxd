...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of entropyxd instances
	entropyxd1, entropyxd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := entropyxd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In entropyxd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, entropyxd2)
	}

	// Connect the two entropyxds. This should trigger sync
	// between the two nodes
	connect(t, entropyxd1, entropyxd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := entropyxd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
