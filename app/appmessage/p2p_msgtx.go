...
// Outpoint defines a entropyx data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new entropyx transaction outpoint point with the
...
// TxIn defines a entropyx transaction input.
type TxIn struct {
...
// NewTxIn returns a new entropyx transaction input with the provided
...
// TxOut defines a entropyx transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new entropyx transaction output with the provided
...
// MsgTx implements the Message interface and represents a entropyx tx message.
