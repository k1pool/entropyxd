...
import (
	...
	"github.com/k1pool/entropyxd/infrastructure/os/execenv"
	"github.com/k1pool/entropyxd/infrastructure/os/limits"
	"github.com/k1pool/entropyxd/infrastructure/os/signal"
	"github.com/k1pool/entropyxd/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "entropyxdsvc",
	DisplayName: "Entropyxd Service",
	Description: "Downloads and stays synchronized with the Entropyx blockDAG and " +
		"provides DAG services to applications.",
}

type entropyxdApp struct {
	cfg *config.Config
}

// StartApp starts the entropyxd app, and blocks until it finishes running
func StartApp() error {
...
func (app *entropyxdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start entropyxd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down entropyxd...")
		...
		log.Infof("Entropyxd shutdown complete")
	}()
