...
// long-running entropyxdMain (which is the real meat of entropyxd), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start entropyxdMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once entropyxd is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("entropyxdMain-windows", func() {
...
// logServiceStart logs information about entropyxd when the main server has
