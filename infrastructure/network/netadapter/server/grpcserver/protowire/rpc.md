...
Clients are expected to build RequestMessages and wrap them in EntropyxdMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a EntropyxdMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a EntropyxdMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network entropyxd is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which entropyx address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether entropyxd thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when entropyxd is not synced. That is because when entropyxd isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known entropyxd addresses in the
...
currently connected to this entropyxd.
...
| isOutbound | [bool](#bool) |  | Whether this entropyxd initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this entropyxd |
...
AddPeerRequestMessage adds a peer to entropyxd&#39;s outgoing connection list.
This will, in most cases, result in entropyxd connecting to said peer.
...
parent chain from some startHash to this entropyxd&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this entropyxd.
...
ShutDownRequestMessage shuts down this entropyxd.
...
This call is only available when this entropyxd was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given entropyxd addresses
