...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-entropyxd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "entropyxd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of entropyxd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-entropyxd.conf")
