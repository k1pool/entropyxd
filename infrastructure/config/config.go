...
const (
	defaultConfigFilename      = "entropyxd.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "entropyxd.log"
	defaultErrLogFilename      = "entropyxd_err.log"
	...
	sampleConfigFilename    = "sample-entropyxd.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for entropyxd.
	DefaultAppDir = util.AppDir("entropyxd", false)
...
//go:embed sample-entropyxd.conf
...
// Flags defines the configuration options for entropyxd.
//
...
type Flags struct {
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 39138, testnet: 16210)"`
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in ENX/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for entropyxd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default entropyxd configuration
...
//
// The above results in entropyxd functioning properly without any config settings
...
// createDefaultConfig copies the file sample-entropyxd.conf to the given destination path,
