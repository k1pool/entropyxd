...
func (s *server) BumpFee(_ context.Context, request *pb.BumpFeeRequest) (*pb.BumpFeeResponse, error) {
	...
	if len(domainTx.Outputs) == 0 || len(domainTx.Outputs) > 2 {
		return nil, errors.Errorf("entropyxwallet supports only transactions with 1 or 2 outputs in transaction %s, but this transaction got %d", request.TxID, len(domainTx.Outputs))
	}
	...
	selectedUTXOs, spendValue, changeSompi, err := s.selectUTXOsWithPreselected([]*walletUTXO{maxUTXO}, allowUsed, domainTx.Outputs[0].Value, false, newFeeRate, maxFee, burnFee, fromAddresses, nil, nil)
	if err != nil {
		return nil, err
	}
