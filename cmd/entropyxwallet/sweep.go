...
import (
	...
	"github.com/k1pool/entropyxd/cmd/entropyxwallet/utils"
...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libentropyxwallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libentropyxwallet.EntropyxwalletdUTXOsTolibentropyxwalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatKas(splitTransactions[i].Outputs[0].Value), " ENX")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatKas(totalExtracted), " ENX")
