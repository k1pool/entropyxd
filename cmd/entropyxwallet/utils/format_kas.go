...
// FormatKas takes the amount of sompis as uint64, and returns amount of ENX with 8  decimal places
func FormatKas(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SompiPerEntropyx)
	}
	return res
}
...
func KasToSompi(amount string) (uint64, error) {
	err := validateENXAmountFormat(amount)
	...
	if constants.SompiPerEntropyx%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerEntropyx is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerEntropyx))
	decimalStr := ""
...
func validateENXAmountFormat(amount string) error {
