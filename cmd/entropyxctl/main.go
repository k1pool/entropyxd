...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		entropyxdMessage, err := client.Post(&protowire.EntropyxdMessage{Payload: &protowire.EntropyxdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := entropyxdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	entropyxdMessage := &protowire.EntropyxdMessage{}
	err := protojson.Unmarshal([]byte(response), entropyxdMessage)
	if err != nil {
	...
	return marshalOptions.Format(entropyxdMessage)
}
