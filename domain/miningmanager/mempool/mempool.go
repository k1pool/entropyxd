...
import (
	...
	"github.com/k1pool/entropyxd/domain/consensusreference"

	"github.com/k1pool/entropyxd/domain/consensus/model/externalapi"
	miningmanagermodel "github.com/k1pool/entropyxd/domain/miningmanager/model"
)
...
func (mp *mempool) BlockCandidateTransactions() []*externalapi.DomainTransaction {
	...
	for _, tx := range readyTxs {
		if len(tx.Outputs) > len(tx.Inputs) {
			...
			if !hasCoinbaseInput && numExtraOuts > 2 && tx.Fee < uint64(numExtraOuts)*constants.SompiPerEntropyx {
				log.Debugf("Filtered spam tx %s", consensushashing.TransactionID(tx))
				continue
			}

			if hasCoinbaseInput || tx.Fee > uint64(numExtraOuts)*constants.SompiPerEntropyx {
