...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("entropyxd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("entropyxd datadir: %s", dataDir)

	entropyxdCmd, err := common.StartCmd("ENTROPYXD",
		"entropyxd",
	...
	spawn("startNode-entropyxdCmd.Wait", func() {
		err := entropyxdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("entropyxdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes entropyxd closes ungracefully
				log.Errorf("entropyxdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(entropyxdCmd, "entropyxdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	entropyxMinerCmd, err := common.StartCmd("MINER",
		"entropyxminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("entropyx-miner-Cmd.Wait", func() {
		err := entropyxMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(entropyxMinerCmd, "entropyxMinerCmd")
	return nil
}
