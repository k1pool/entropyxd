...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("entropyxd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("entropyxd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("ENTROPYXD-SYNCER",
		"entropyxd",
	...
	syncedCmd, err := common.StartCmd("ENTROPYXD-SYNCED",
		"entropyxd",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
