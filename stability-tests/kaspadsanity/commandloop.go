...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("entropyxdsanity-entropyxd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("entropyxd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "ENTROPYXD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "ENTROPYXD-STDERR")
